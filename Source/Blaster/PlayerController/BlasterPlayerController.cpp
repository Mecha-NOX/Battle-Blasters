...
void ABlasterPlayerController::SetHUDWeaponIcon(UTexture2D* WeaponIconTexture)
{
	...
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->WeaponIcon->SetBrushFromTexture(WeaponIconTexture, true);
		BlasterHUD->CharacterOverlay->WeaponIcon->SetVisibility(ESlateVisibility::Visible);
	}
}

void ABlasterPlayerController::ClearHUDWeaponIcon()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
